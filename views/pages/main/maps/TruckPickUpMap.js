...
import { getCoordinatesFromAddress, getRoutesFromCoordinates } from '../action';
...
export default function TruckPickUpMap(props) {
    ...
    const [pick_drop_status, setpick_drop_status] = useState(true);
    ...
    useEffect(() => {
        if (location && location["coords"])
            if (location["coords"].longitude) {
                setfrom_address_coordinates([location["coords"].longitude, location["coords"].latitude])
            }
        if (shipmentDetails.pickup_address) {
            get_coordinates_from_address(shipmentDetails.pickup_address, (coordinates) => { setto_address_coordinates(coordinates) })
        }

    }, [shipmentDetails.pickup_address, location])
    ...
    useEffect(() => {
        if (from_address_coordinates && to_address_coordinates && from_address_coordinates.length > 0 && to_address_coordinates.length > 0) {
            ...
            getRoutesFromCoordinates(from_address_coordinates, to_address_coordinates,
                (successResponse) => {
                    if (successResponse.code == 'InvalidInput') {
                        alert(successResponse.message + ". Your current location is too far from the shipment location.")
                    }
                    else {
    ...
    return (
        ...
        <View style={{ height: SCREEN.HEIGHT, width: SCREEN.WIDTH, backgroundColor: "#252746" }}>
            <Header navigation={props.navigation} headerName={"Truck PickUp Routes"}
            ...
            />
            <ConfirmationModal visible={visible} headerText={"Are you sure , You reached the origin point ?"} onClose={() => setvisible(false)} onSubmit={() => { props.navigation.navigate("ContainerDetails") }} />
            ...
            <View style={styles.container}>
...

                {
                    ...
                    <MapView style={styles.map} region={mapRegion} mapType='mutedStandard' >
                        ...
                        {truckRoutes && truckRoutes[0] &&
                            <Marker coordinate={mapRegion} image={require("../../../Image/box-truck.png")} style={{ transform: `rotate(${calculateHeading({ latitude: truckRoutes[0][1], longitude: truckRoutes[0][0] }, { latitude: truckRoutes[1][1], longitude: truckRoutes[1][0] })}deg)` }} />
                        }
                ...
                <View style={{ position: "absolute", zIndex: 5, bottom: 10, right: 10 }}>
                    <TouchableOpacity style={{ ...styles.tabTouch, backgroundColor: "#5d4fbf", borderRadius: 15, elevation: 5 }} onPress={() => { setvisible(true) }} >
                        <Text style={{ color: "#fff", fontSize: 17, fontFamily: "serif" }}>CHECK IN</Text>
                    </TouchableOpacity>
                </View>
            </View >
        </View>
